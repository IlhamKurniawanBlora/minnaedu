<template>
<footer class="relative bg-blueGray-200 pt-8 pb-6">
  
  <div class="container mx-auto px-4">
    <div class="flex flex-wrap text-left lg:text-left">
      <div class="w-full lg:w-6/12 px-4">
        <h4 class="text-3xl font-semibold text-blueGray-700">Let's keep in touch!</h4>
        <h5 class="text-lg mt-0 mb-2 text-blueGray-600">
          We're here to help and answer any question you might have. We look forward to hearing from you.
        </h5>
        <div class="mt-6 lg:mb-0 mb-6 flex flex-wrap lg:justify-start md:justify-start justify-center gap-5">
          <a href="https://api.whatsapp.com/send?phone=6287764848768" target="_blank" rel="noopener noreferrer" class="bg-white text-green-400 shadow-lg font-normal h-10 w-10 items-center justify-center align-center flex rounded-full outline-none focus:outline-none mr-2">
              <LogoWhatsapp />
            </a>
          <a href="https://www.instagram.com/minnaedu/" target="_blank" rel="noopener noreferrer" class="bg-white text-red-400 shadow-lg font-normal h-10 w-10 items-center justify-center align-center flex rounded-full outline-none focus:outline-none mr-2">
              <LogoInstagram />
            </a>
          <a href="https://www.instagram.com/minnaedu/" target="_blank" rel="noopener noreferrer" class="bg-white text-blue-400 shadow-lg font-normal h-10 w-10 items-center justify-center align-center flex rounded-full outline-none focus:outline-none mr-2">
              <LogoFacebook />
            </a>
            
              
        </div>
      </div>
      <div class="w-full lg:w-6/12 px-4">
        <div class="flex flex-wrap items-top items-between justify-between mb-6">
          <div class="w-1/2 lg:w-4/12 px-4 ml-auto">
            <span class="block uppercase text-blueGray-500 text-sm font-semibold mb-2">Menu Links</span>
            <ul class="list-unstyled">
              <li>
                <a class="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="#">About Us</a>
              </li>
              <li>
                <a class="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="#">Testimoial</a>
              </li>
              <li>
                <a class="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="#">Galery</a>
              </li>
              <li>
                <a class="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm" href="#">Consultation</a>
              </li>
            </ul>
          </div>
          <div class="w-1/2  px-4">
            <span class="block uppercase text-blueGray-500 text-sm font-semibold mb-2">Contact Us</span>
            <ul class="list-unstyled">         
            <li class="flex items-center">
              <a class="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm flex items-center" href="https://maps.app.goo.gl/2dsG33pXr3Hsmt5W7">
              <LogoMap />
              Minna Education
              </a>
            </li>
            <li class="flex items-center">
              <a class="text-blueGray-600 hover:text-blueGray-800 font-semibold block pb-2 text-sm flex items-center" href="#">
               <LogoCall />
                0877 6484 8768
              </a>
            </li>
            

            </ul>
          </div>
        </div>
      </div>
    </div>
    <hr class="my-6 border-blueGray-300">
    <div class="flex flex-wrap items-between justify-between w-full px-4">
        <div class="text-sm text-blueGray-500 font-semibold py-1">
            Copyright © <span id="get-current-year">2024</span>
        </div>
        <div class="text-sm text-blueGray-500 font-semibold py-1">
        <a href="#" class="text-blueGray-500 hover:text-blueGray-800">All Reserved</a>
        </div>
    </div>
  </div>
</footer>
</template>